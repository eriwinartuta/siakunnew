...
import BaganAkunStandar5 from "./BASSub5";

const BaganAkunStandar4 = ({ record, dispatch, useSelector }) => {
  const { datalistakun4, akun4byid, editakun4, postakun4, akun4bysub3 } = useSelector(
  ...
  const column = [
    {
      ...
      render: (text, record, index) => {
        ...
        return <Tag style={{ fontSize: 16,  padding: 5, margin:5 }} color="#F23D5D">{text}</Tag>;
      },
    },
    {
      title: "Deskripsi",
      dataIndex: "uraian_akun_4",
      key: "uraian_akun_4",
      filterDropdown: ({
        setSelectedKeys,
        selectedKeys,
      ...
      onFilter: (value, record) => {
        return record.uraian_akun_4.toLowerCase().includes(value.toLowerCase());
      },
      render: (text, record, index) => {
        if (record.uraian_akun_4 === null) {
          return (
        ...
        <Tabel
          ...
          expandIcon={({ expanded, onExpand, record }) => {
            ...
            return expanded ? (
              ...
              <PlusCircleOutlined
                onClick={() => {
                  setExpandKey([record.kode_akun_4]);
                  dispatch(fetchAkun5bySub4(record.kode_akun_4));
...
export default BaganAkunStandar4;
