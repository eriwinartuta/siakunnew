...
import ChartOfAccount4 from "./COA4";
import { fetchCOA4bySub3 } from "../../../store/coa/action";

const ChartOfAccount3 = ({ record, dispatch, useSelector }) => {
  const { coa3bysub2 } = useSelector((state) => state.reducerCOA);
  ...
  const column = [
    {
      ...
      dataIndex: "kode_coa_3",
      key: "kode_coa_3",
      width: 50,
      align: "center",
      render: (text, record, index) => {
        if (record.uraian_coa_3 === null) {
          return <div></div>;
        }
        return (
          <Tag style={{ fontSize: 16, padding: 5, margin: 5 }} color="#1CD386">
            {text}
          </Tag>
        );
      },
    },
    {
      ...
      onFilter: (value, record) => {
        return record.uraian_coa_3.toLowerCase().includes(value.toLowerCase());
      },
      render: (text, record, index) => {
        // if (record.uraian_coa_3 === null) {
        //   return (
        ...
        return (
          <div>
            ...
            <Space> {record.uraian_coa_3} </Space>
          </div>
        );
      },
    },
          ...
          expandIcon={({ expanded, onExpand, record }) => {
            ...
            return expanded ? (
              <MinusCircleOutlined
                onClick={() => {
                  setExpandKey([]);
                  dispatch(fetchCOA4bySub3(0));
                }}
              />
            ) : (
              <PlusCircleOutlined
                onClick={() => {
                  setExpandKey([record.kode_coa_3]);
                  dispatch(fetchCOA4bySub3(record.kode_coa_3));
                }}
              />
            );
          }}
...
export default ChartOfAccount3;
