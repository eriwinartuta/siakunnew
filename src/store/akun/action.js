...
export const fetchBAS = () => async (dispatch) => {
  const response = await dispatch(
    axiosGet(BASE_URL_AKUN + BASE_PATH_AKUN.nested_all)
  );
  dispatch({ type: type.NESTED_ALL, value: response.Akun1 });
  dispatch({ type: type.DATA_LIST_AKUN_1, value: response.Akun1 });
};

export const fetchSaldoAwal = (val) => async (dispatch) => {
  const response = await dispatch(
    axiosGet(EBUDGETING_APP + BASE_PATH_AKUN.get_saldounit + val )
  );
  dispatch({ type: type.LIST_SALDO_AWAL, value: response.values });
};

export const fetchAkun1 = () => async (dispatch) => {
  //console.log("test masuk")
  const response = await dispatch(
    axiosGet(BASE_URL_AKUN_INTERNAL + BASE_PATH_AKUN.get_all_sub_1)
  );
